...
const getDayName = (num) => ['DOM', 'LUN', 'MAR', 'MIE', 'JUE', 'VIE', 'SAB'][num] || '?';
...
export default function App() {
  ...
  const isStudent = () => !isMaster() && !isProfesor() && !isAdministrative();
...
// ==========================================
  ...
  const handleCreateClasses = async () => {
    ...
    if (!formData.coach) return Alert.alert("Error", "Selecciona un Coach general");
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* CLASES (SISTEMA DE RESERVAS & BOXES) - VERSIÓN FINAL CORREGIDA */}
        {view === 'clases' && (
            <View>
                ...
                {/* GRILLA DE HORARIOS */}
                {(() => {
                    const currentBox = selectedBox || 'Principal';
                    const clasesFiltradas = (clases || []).filter(c => 
                        (currentBox === 'Principal' ? (!c.box_nombre || c.box_nombre === 'Principal') : c.box_nombre === currentBox)
                    );

                    const slots = [];
                    for (let h = 7; h <= 21; h += 1) slots.push(h);
                    ...
                    return slots.map((time) => {
                        ...
                        const cl = slotReal?.clase;
                        const cupoActual = cl ? (reservas || []).filter(r => 
                            String(r.clase_id) === String(cl.id) && 
                            Number(r.dia_semana) === Number(selectedDayCalendar) && 
                            parseFloat(r.horario).toFixed(1) === parseFloat(slotReal.horario).toFixed(1)
                        ).length : 0;
                ...
                <View>
                    ...
                    <View style={{ marginBottom: 20 }}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }}>
                            {['Todos', 'Efectivo', 'Transferencia', 'Mercado Pago', 'Debito', 'Credito'].map((metodo) => (
                                <TouchableOpacity 
                                    key={metodo} 
                                ...
                                >
                                    <Text style={{ 
                                        color: metodoFilter === metodo ? 'white' : '#71717a', 
                                        fontSize: 10, 
                                        fontWeight: '900' 
                                    }}>
                                        {metodo.toUpperCase()}
