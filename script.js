    ...
    // --- CONFIGURACIÓN VISUAL (MODO IMPACTO TOTAL Y CORRECCIÓN DE SCROLL) ---
    // Aumentamos auto-rows a 140px para que el texto gigante respire
    cal.className = "calendar-container h-[700px] overflow-auto custom-scrollbar grid grid-cols-[60px_repeat(6,minmax(180px,1fr))] auto-rows-[140px] gap-[1px] bg-black p-1 rounded-3xl snap-none border border-white/10"; 
    ...
    // Header HORA con borde grueso inferior para evitar que se vea lo de atrás
    let headersHTML = `
        <div class="cal-header sticky top-0 left-0 z-[100] bg-[#0a0a0a] flex items-center justify-center font-black italic text-[10px] text-white/30 p-2 border-b-4 border-red-600 rounded-tl-2xl shadow-xl">
            HORA
        </div>
    `;
    ...
    diasNombres.forEach((nombreDia, index) => {
        ...
        headersHTML += `
            <div class="cal-header sticky top-0 z-[90] ${bgClass} ${roundedClass} p-2 flex flex-col items-center justify-center border-b-4 ${esHoy ? 'border-white' : 'border-red-600'} transition-colors shadow-2xl">
                <span class="text-[10px] font-black uppercase tracking-widest leading-none mb-1 ${subTextClass}">${mesNombre}</span>
                <h4 class="text-3xl font-black italic leading-none mb-1 ${textClass}">${numeroDia}</h4>
                <span class="text-[12px] font-bold uppercase ${subTextClass}">${nombreDia}</span>
            </div>
        `;
    });
    ...
    for(let h=7; h<=21.5; h+=0.5) {
        ...
        hourLabel.className = "cal-cell sticky left-0 z-30 bg-[#0d0d0d] flex items-center justify-center font-black text-[11px] text-white/50 border-r border-white/20 min-h-[140px] shadow-2xl";
        ...
        for(let d=1; d<=6; d++) {
            ...
            cell.id = cellId;
            cell.className = `cal-cell relative min-h-[140px] border-b border-r border-white/5 hover:bg-white/10 transition-colors ${isClosed ? 'bg-stripes-gray opacity-30 pointer-events-none' : ''}`;
        ...
        state.clases.forEach(c => {
            ...
            horarios.forEach(slot => {
                ...
                if (cell) {
                    ...
                    badge.className = "absolute inset-1.5 rounded-2xl shadow-2xl transition-all hover:scale-[1.03] active:scale-95 cursor-pointer flex flex-col items-center justify-center p-2 z-10 class-badge border-2 border-black/10"; 
                    ...
                    badge.innerHTML = `
                        <div class="flex flex-col items-center justify-center h-full w-full px-2 gap-2">
                            <span class="text-[18px] leading-tight font-black uppercase italic ${colores.text} text-center truncate w-full drop-shadow-lg">
                                ${c.nombre}
                            </span>
                            <span class="text-[24px] font-black ${colores.sub} uppercase italic leading-none truncate w-full text-center tracking-tighter">
                                PROF.: ${slot.coach || 'STAFF'}
                            </span>
                            <div class="mt-2 ${colores.bg} px-6 py-2 rounded-2xl text-[24px] font-black leading-none border-2 border-black/10 ${estaLleno ? 'bg-red-600 text-white animate-pulse shadow-[0_0_25px_rgba(255,0,0,0.8)]' : colores.text}">
                                ${cupoActual} / ${cupoMax}
                            </div>
                        </div>
                    `;
