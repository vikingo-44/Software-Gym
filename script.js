		...
		async function renderCalendar() {
            ...
            // --- CONFIGURACIÓN VISUAL PROFESIONAL (COMPACTA) ---
            // Ajustamos el minmax a 130px y gridAutoRows a 70px para que entre en monitor
            cal.className = "calendar-container h-[700px] overflow-auto custom-scrollbar grid grid-cols-[80px_repeat(6,minmax(130px,1fr))] gap-[1px] bg-white/5 p-1 rounded-3xl border border-white/10 shadow-2xl"; 
            ...
            let headersHTML = `
                <div class="cal-header sticky top-0 left-0 z-[120] bg-black flex items-center justify-center font-black italic text-[9px] text-red-600 p-2 border-b-4 border-red-600 rounded-tl-2xl">
                    HORA
                </div>
            `;
            ...
            diasNombres.forEach((nombreDia, index) => {
                ...
                headersHTML += `
                    <div class="cal-header sticky top-0 z-[110] ${bgClass} ${roundedClass} p-1 flex flex-col items-center justify-center border-b-4 ${borderClass} transition-colors">
                        <span class="text-[8px] font-black uppercase tracking-widest leading-none ${subTextClass}">${mesNombre}</span>
                        <h4 class="text-lg font-black italic leading-none ${textClass}">${numeroDia}</h4>
            ...
            for(let h=7; h<=21.5; h+=0.5) {
                ...
                hourLabel.className = "cal-cell sticky left-0 z-40 bg-[#0a0a0a] flex items-center justify-center font-black text-[10px] text-white/30 border-r border-white/5";
                        ...
                        if (cell) {
                            ...
                            badge.innerHTML = `
                                <div class="flex flex-col items-center justify-center h-full w-full px-1">
                                    <span class="text-[9px] leading-tight font-black uppercase italic ${colores.text} text-center truncate w-full">
                                        ${c.nombre}
                                    </span>
                                    <span class="text-[10px] font-black ${colores.sub} uppercase italic leading-none truncate w-full text-center mt-0.5">
                                        Prof: ${slot.coach || 'STAFF'}
                                    </span>
                                    <div class="mt-1 px-2 py-0.5 rounded-lg text-[9px] font-black bg-black/20 ${estaLleno ? 'bg-red-600 text-white animate-pulse' : colores.text}">
                                        ${cupoActual}/${cupoMax}
                                    </div>
                                </div>
                            `;
