    ...
    // --- CONFIGURACIÓN VISUAL (MODO IMPACTO TOTAL) ---
    // Sincronizamos con el CSS: 160px de altura para que el texto gigante respire.
    ...
    // CABECERA HORA: Fondo negro sólido + Borde inferior ROJO GRUESO (Máscara para el scroll)
    let headersHTML = `
    ...
    diasNombres.forEach((nombreDia, index) => {
        ...
        const bgClass = esHoy ? "bg-red-600 text-black" : "bg-[#0a0a0a] text-gray-400";
        ...
        headersHTML += `
            <div class="cal-header sticky top-0 z-[110] ${bgClass} ${roundedClass} p-3 flex flex-col items-center justify-center border-b-8 ${borderClass} shadow-2xl">
    ...
    for(let h=7; h<=21.5; h+=0.5) {
        ...
        for(let d=1; d<=6; d++) {
            ...
            if (isAdmin && !isClosed) {
                cell.ondragover = (e) => { e.preventDefault(); cell.classList.add('bg-red-600/10'); };
                ...
                cell.ondrop = async (e) => {
                    ...
                    const res = await apiFetch(`/clases/${claseId}/move`, 'PUT', { old_dia: parseInt(oldDia), old_horario: parseFloat(oldHorario), new_dia: newDia, new_horario: newHorario });
                    if (!res.error) { showVikingToast("¡Turno Reubicado!"); state.clases = await apiFetch('/clases'); renderCalendar(); }
                };
            }
    ...
    if(state.clases && Array.isArray(state.clases)){
        state.clases.forEach(c => {
            ...
            const getTextColorClass = (hexColor) => {
                ...
                return (yiq >= 128) ? { text: 'text-black', sub: 'text-black/70', bg: 'bg-black/20' } : { text: 'text-white', sub: 'text-white/80', bg: 'bg-white/20' };
            };
            ...
            horarios.forEach(slot => {
                ...
                if (cell) {
                    ...
                    const cupoActual = reservasArray.filter(r => String(r.clase_id) === String(c.id) && Number(r.dia_semana) === Number(slot.dia) && Number(r.horario) === Number(slot.horario)).length;
                    const estaLleno = cupoActual >= cupoMax;

                    const badge = document.createElement('div');
                    badge.className = "absolute inset-2 class-badge shadow-2xl flex flex-col items-center justify-center p-4 z-10 border-4 border-black/20"; 
                    badge.style.backgroundColor = c.color || '#FF0000';
                    badge.style.height = "calc(100% - 16px)"; 
                    
                    if (isAdmin) {
                        badge.draggable = true; 
                        badge.ondragstart = (e) => { e.dataTransfer.setData("claseId", c.id); e.dataTransfer.setData("oldDia", slot.dia); e.dataTransfer.setData("oldHorario", slot.horario); badge.classList.add('opacity-40'); };
                    ...
                    badge.innerHTML = `
                        <div class="flex flex-col items-center justify-center h-full w-full px-2 gap-4">
                            ...
                            <!-- Nombre de Profesor GIGANTE + PREFIJO "Prof.:" -->
